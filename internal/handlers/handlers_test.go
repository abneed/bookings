...
func TestRepository_PostReservation(t *testing.T) {
	...
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=Abneed")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Rodriguez")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=abneed.rodriguez@gmail.com")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=+5218671487159")
	...
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2050-01-2")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=Abneed")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Rodriguez")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=abneed.rodriguez@gmail.com")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=+5218671487159")
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Reservation handler failed when trying to fail inserting reservation: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Reservation handler failed when trying to fail inserting restriction: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
}
...
func TestRepository_PostAvailability(t *testing.T) {
	...
	*****************************************/
	...
	// create our request
	req, _ := http.NewRequest("POST", "/search_availability", strings.NewReader(reqBody))
...
func TestRepository_AvailabilityJSON(t *testing.T) {
	...
	*****************************************/
	...
	if j.OK || j.Message != "Error querying database" {
		t.Error("Got availability when start date is not valid")
	}

	/*****************************************
	// fifth case -- start date not valid
	*****************************************/
	// create our request body
	reqBody = "start=invalid"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end=2050-01-02")
	...
	// since we specified a start date is not valid, we expect no availability
	if j.OK || j.Message != "Start date is not valid" {
		t.Error("Start date is not valid")
	}

	/*****************************************
	// sixth case -- end date not valid
	*****************************************/
	// create our request body
	reqBody = "start=2050-01-01"
	...
	// since we specified a start date is not valid, we expect no availability
	if j.OK || j.Message != "End date is not valid" {
		t.Error("Got availability when end date is not valid")
	}
}
...
func TestRepository_BookRoom(t *testing.T) {
	...
	*****************************************/
	...
	/*****************************************
	// third case -- start date not valid
	*****************************************/
	reservation = models.Reservation{
		RoomID: 1,
		Room: models.Room{
