...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	htmlMessage = fmt.Sprintf(`
		<strong>Reservation Notifcation</strong><br>
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	startDate, err := time.Parse(layout, sd)
	if err != nil {
		// can't parse start date, so return appropriate json
		resp := jsonResponse{
			OK:      false,
	...
	endDate, err := time.Parse(layout, ed)
	if err != nil {
		// can't parse end date, so return appropriate json
		resp := jsonResponse{
			OK:      false,
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	m.App.Session.Put(r.Context(), "flash", "Changes saved")
	http.Redirect(w, r, fmt.Sprintf("/admin/reservations-calendar?=%d&m=%d", year, month), http.StatusSeeOther)
}
